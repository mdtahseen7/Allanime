...
var ALLANIME_API = `https://api.${ALLANIME_BASE}`;
var ALLANIME_KEY = null;
...
async function decrypt(blob) {
  try {
    const raw2 = Uint8Array.from(atob(blob), (c) => c.charCodeAt(0));
    const iv = raw2.slice(1, 13);
    const ctLen = raw2.length - 13 - 16;
    const ciphertext = raw2.slice(13, 13 + ctLen);
...
__name(decodeProviderId, "decodeProviderId");
...
__name(getEpisodesList, "getEpisodesList");
async function getEpisodeUrl(showId, epNo, mode = "sub", quality = "best") {
  ...
  try {
    ...
    if (respData.tobeparsed) {
      ...
      const raw2 = JSON.stringify(respData.episode.sourceUrls);
      const cleaned = raw2.replace(/\\u002F/g, "/").replace(/\\/g, "");
...
__name(getEpisodeUrl, "getEpisodeUrl");
...
__name(getEpisodeInfo, "getEpisodeInfo");
...
app.get("/episodes/:id", async (c) => {
  ...
  try {
    const episodes = await getEpisodesList(id, mode);
...
app.get("/episode_url", async (c) => {
  ...
  try {
    const url = await getEpisodeUrl(id, epNo, mode, quality);
...
app.get("/episode_info", async (c) => {
  ...
  if (!id || !epNo) return c.json({ error: "Missing show_id or ep_no" }, 400);
  try {
    const info3 = await getEpisodeInfo(id, epNo);
    if (!info3) return c.json({ error: "Episode info not found" }, 404);
    return c.json(info3);
  } catch (e) {
    return c.json({ error: e.message }, 500);
  }
...
// .wrangler/tmp/bundle-coOlLE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-coOlLE/middleware-loader.entry.ts
